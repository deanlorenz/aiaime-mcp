...
// aiaimeHome returns the root of all aiaime local state.
func aiaimeHome() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".aiaime")
}

// subsPath returns the path to a session's subscription list file.
func subsPath(busID, sessionID string) string {
	return filepath.Join(aiaimeHome(), "subs", busID, sessionID+".json")
}
...
func MarkerPath(busID, sessionID, topic string) string {
	...
	return filepath.Join(aiaimeHome(), "markers", busID, sessionID, safe+".marker")
}
...
func CursorPath(busID, sessionID, topic string) string {
	safe := topicToFilename(topic)
	return filepath.Join(aiaimeHome(), "cursors", busID, sessionID, safe+".cursor")
}
...
// always overwrites the filter). Used by async aiaime_ask_user.
