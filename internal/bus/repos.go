...
func ResolveBusID(cwd string) (string, error) {
	...
	if bestID == "" {
		return "", fmt.Errorf("no aiaime registration found for %s\nrun: aiaime-setup <bus_id>", abs)
	}
	return bestID, nil
}

// RegisterRepo writes root → busID into ~/.aiaime/repos.json.
// Called by aiaime-setup.
