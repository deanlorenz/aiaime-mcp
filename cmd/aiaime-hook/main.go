// aiaime-hook is the PostToolBatch hook binary. Registered once globally in
// ~/.claude/settings.json; fires after every batch of tool calls.
//
// Fast path (common): reads subscription list and markers from ~/.aiaime/,
...
func main() {
	...
	for _, topic := range subs.Topics {
		...
		if err != nil {
			// Don't advance cursor on fetch error — retry next turn.
			continue
		}
...
func formatContext(messages []schema.Message) string {
	...
	b.WriteString("[aiaime] New messages:\n")
