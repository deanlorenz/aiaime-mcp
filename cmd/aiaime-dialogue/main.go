// aiaime-dialogue is an interactive CLI tool for developers to interact
// with AI agents via aiaime topics (by default, listening on user.in and
...
// state: 0 = none/clear, 1 = normal, 2 = error, 3 = indeterminate, 4 = warning
...
// setTerminalTitle updates the window/tab title across xterm, VS Code, and tmux.
...
// renderSimpleMarkdown renders headers, bold, code blocks, and lists with ANSI formatting.
func renderSimpleMarkdown(body string) string {
	...
	for _, line := range lines {
		...
		if strings.HasPrefix(trimmed, "### ") {
			out = append(out, fmt.Sprintf("\033[1;34m### %s\033[0m", strings.TrimPrefix(trimmed, "### ")))
			continue
		} else if strings.HasPrefix(trimmed, "## ") {
			out = append(out, fmt.Sprintf("\033[1;36m## %s\033[0m", strings.TrimPrefix(trimmed, "## ")))
			continue
		} else if strings.HasPrefix(trimmed, "# ") {
			out = append(out, fmt.Sprintf("\033[1;35m# %s\033[0m", strings.TrimPrefix(trimmed, "# ")))
			continue
		}
		...
		if matched, _ := regexp.MatchString(`^\d+\.\s`, trimmed); matched {
			...
			if len(parts) == 3 {
				item := boldRegex.ReplaceAllString(parts[2], "\033[1m$1\033[22m")
				item = codeRegex.ReplaceAllString(item, "\033[33m$1\033[39m")
...
// newReadline creates a readline instance reading from /dev/tty.
...
// readUserReply reads input using readline. Trailing \ continues to next line.
...
func main() {
	...
	sessionID := flag.String("session", "user", "user session identity")
	flag.Parse()
	...
	if busID == "" {
		...
		if err != nil {
			log.Fatalf("aiaime-dialogue: %v", err)
		}
	}
	...
	idleTitle := fmt.Sprintf("aiaime-dialogue [%s]", busID)
	setTerminalTitle(idleTitle)
	setTerminalProgress(0, 0)

	fmt.Printf("\033[1;36m╔════════════════════════════════════════════════════════════╗\033[0m\n")
	fmt.Printf("\033[1;36m║                 AIAIME INTERACTIVE DIALOGUE                ║\033[0m\n")
	fmt.Printf("\033[1;36m╚════════════════════════════════════════════════════════════╝\033[0m\n")
	...
	for {
		...
		case <-ctx.Done():
			setTerminalTitle(idleTitle)
			setTerminalProgress(0, 0)
			fmt.Println("\nExiting aiaime-dialogue.")
			return
		case msg := <-msgChan:
			attentionTitle := fmt.Sprintf("💬 [ACTION REQUIRED] aiaime-dialogue [%s]", busID)
