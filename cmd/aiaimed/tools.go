...
func registerTools(server *mcp.Server, js jetstream.JetStream, busID string) {
	...
	mcp.AddTool(server, &mcp.Tool{
		Name:        "aiaime_ask_user",
		Description: "Ask the user an interactive question via aiaime dialogue (user.in / user.out) and wait for reply.",
...
func agentName() string {
	if a := os.Getenv("AIAIME_AGENT_NAME"); a != "" {
		return a
	}
	return "unknown"
}
...
func askUserSync(ctx context.Context, js jetstream.JetStream, busID string, args askUserArgs) (*mcp.CallToolResult, askUserResult, error) {
	...
	if _, err := publishQuestion(ctx, js, busID, args.FromSession, args.Prompt, args.Refs); err != nil {
		return nil, askUserResult{}, err
	}
