// aiaime-pub is a minimal CLI for publishing a single message to an
// aiaime topic. Intended for manual testing and shell scripting.
//
// Usage:
//
//	aiaime-pub -topic user.in -body "hello" [-kind question] [-agent bob] [-session test]
package main
...
func main() {
	...
	if *topic == "" || *body == "" {
		fmt.Fprintln(os.Stderr, "usage: aiaime-pub -topic <topic> -body <body> [-kind <kind>] [-agent <agent>] [-session <session>]")
		os.Exit(1)
	}
